...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    check_pole('radio', "Не выбран пол", empty($radio) || !preg_match('/^(male|female)$/', $radio));
    ...
    if (!check_pole('language', 'Не выбран язык', empty($language))) {
        try {
            ...
            $dbLangs = $db->prepare("SELECT lang_id, lang FROM langs WHERE lang_id IN ($inQuery)");
    ...
    if (!$error) {
        ...
        if ($log) {
            ...
            $stmt1 = $db->prepare("INSERT INTO users_langs (form_id, lang_id) VALUES (?, ?)");
            foreach ($languages as $row)
                $stmt1->execute([$_SESSION['form_id'], $row['lang_id']]);
            if ($adminLog)
            ...
            try {
                ...
                $stmt = $db->prepare("INSERT INTO users (id, fio, number, email, date_r, male, biography) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$user_id, $fio, $number, $email, $date, $radio, $bio]);
                $fid = $db->lastInsertId();

                $stmt1 = $db->prepare("INSERT INTO users_langs (form_id, lang_id) VALUES (?, ?)");
                foreach ($languages as $row)
                    $stmt1->execute([$fid, $row['lang_id']]);
            } catch (PDOException $e) {
                print ('Error : ' . $e->getMessage());
                exit();
            }
    ...
    if ($error && $log) {
        try {
            $dbLangs = $db->prepare("SELECT * FROM users WHERE id = ?");
            $dbLangs->execute([$uid]);
            $user_inf = $dbLangs->fetchAll(PDO::FETCH_ASSOC)[0];

            $form_id = $user_inf['form_id'];
            ...
            set_val('date', $user_inf['date_r']);
            set_val('radio', $user_inf['male']);
            set_val('language', $language);
            set_val('bio', $user_inf['biography']);
            set_val('check', "1");
        } catch (PDOException $e) {
            print ('Error : ' . $e->getMessage());
            exit();
